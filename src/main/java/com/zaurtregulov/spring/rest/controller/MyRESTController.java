...
public class MyRESTController {
    ...
    public Employee getEmployee(@PathVariable int id) { // PathVariable нужна для того чтобы читать из url id
        Employee employee = employeeService.getEmployee(id);
        if (employee == null) {
            throw new NoSuchEmployeeException("There is no employee  ID = " +
                    id + " in Database");
        }
        return employee;
    }
